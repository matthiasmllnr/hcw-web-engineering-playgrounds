import { imageExists, showError, toErrorMessage } from '@/utils';
...
export const initBears = async () => {
  try {
    ...
    showError(
      toErrorMessage(err) || 'Failed to load bears. Please try again later.'
    );
  }
};
...
const extractBears = async (wikitext: string) => {
  ...
  const bearPromises = rows.map(async (row) => {
    ...
    const rangeImageName = rangeMatch
      ? rangeMatch[2]?.trim().replace('File:', '')
      : null;
